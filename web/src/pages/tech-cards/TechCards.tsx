...
export default function TechCards() {
  ...
  const handleApprove = async (
    ...
    setConfirmModal({
      ...
      message: isApprove
        ? t("techCards.confirmApprove")
        : t("techCards.confirmReject"),
      ...
      onConfirm: async () => {
        try {
          await techCardsApi.updateApproval(
            currentLocation.id,
            item.id,
            status,
          );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {t("techCards.description")}
          </p>
        </div>
        <Protected
          permission={{ resource: "tech_card_items", action: "create" }}
        >
          <button
            ...
            <SearchableSelect
              ...
              value={
                statusFilter
                  ? {
                      id: statusFilter,
                      name: t(`techCards.status.${statusFilter}`),
                    }
                  : { id: "", name: t("common.all") }
              }
              onChange={(selected: SelectOption | null) =>
                setStatusFilter(
                  selected && selected.id !== "" ? String(selected.id) : "",
                )
              }
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                        {margin > 0 ? (
                          <div>
                            ...
                            <div
                              className={`text-xs ${marginPercent > 30 ? "text-green-600" : marginPercent > 15 ? "text-yellow-600" : "text-red-600"}`}
                            >
