...
export default function InvoiceModal({
  ...
  const loadDropdownData = useCallback(async () => {
    ...
    setLoadingData(true);
    try {
      const [suppliersRes, categoriesRes, sectionsRes, unitsRes] =
        await Promise.all([
          suppliersApi.list({
          ...
          expenseCategoriesApi.listByBusiness(currentLocation.id, {
            is_active: true,
            limit: 1000,
          }),
          expenseSectionsApi.list({
            business_id: currentLocation.id,
            is_active: true,
            limit: 1000,
          }),
  ...
  const getUnitsOptionsForCategory = (categoryId: number): SelectOption[] => {
    const filteredUnits = getFilteredUnitsForCategory(
      categoryId,
      categories,
      units,
    );
  ...
  const handleLineItemChange = (
    ...
    if (field === "quantity" || field === "unit_price") {
      const quantity =
        parseFloat(
          field === "quantity" ? (value as string) : updated[index].quantity,
        ) || 0;
      const unitPrice =
        parseFloat(
          field === "unit_price"
  ...
  const validateForm = (): boolean => {
    ...
    for (let i = 0; i < lineItems.length; i++) {
      ...
      if (!item.category_id || item.category_id === 0) {
        setError(
          t("expenses.invoices.modal.categoryRequired", { line: i + 1 }),
        );
        return false;
      }
      if (!item.quantity || parseFloat(item.quantity) <= 0) {
        setError(
          t("expenses.invoices.modal.quantityRequired", { line: i + 1 }),
        );
        return false;
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        axiosError?.response?.data?.detail || regularError?.message || "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        errorMessage ||
          (isEditing
            ? t("expenses.invoices.modal.updateError")
            : t("expenses.invoices.modal.createError")),
      );
    } finally {
      setIsLoading(false);
    }
                ...
                <Dialog.Panel className="w-full max-w-4xl transform rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-6">
                      ...
                      >
                        {isViewing
                          ? t("expenses.invoices.modal.viewTitle")
                          : isEditing
                            ? t("expenses.invoices.modal.editTitle")
                            : t("expenses.invoices.modal.createTitle")}
                      </Dialog.Title>
                      <button
                        onClick={onClose}
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            <label className="block text-sm font-medium text-gray-700">
                              {t("expenses.invoices.modal.supplier")}{" "}
                            ...
                            <div className="mt-1">
                              <SearchableSelect
                                ...
                                placeholder={t(
                                  "expenses.invoices.modal.selectSupplier",
                                )}
                                searchPlaceholder={t(
                                  "expenses.invoices.modal.searchSupplier",
                                )}
                                noResultsText={t(
                                  "expenses.invoices.modal.noSuppliersFound",
                                )}
                          ...
                          <div>
                            ...
                            <div className="mt-1">
                              <Input
                                ...
                                placeholder={t(
                                  "expenses.invoices.modal.invoiceNumberPlaceholder",
                                )}
                                disabled={isViewing}
                              />
                            </div>
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700">
                              {t("expenses.invoices.modal.invoiceDate")}{" "}
                              <span className="text-red-500">*</span>
                            </label>
                            <div className="mt-1">
                              <Input
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-3">
                            <label className="block text-sm font-medium text-gray-700">
                              {t("expenses.invoices.modal.lineItems")}{" "}
                              <span className="text-red-500">*</span>
                            </label>
                            {!isViewing && (
                            ...
                            {lineItems.map((item, index) => (
                              ...
                              >
                                <div className="col-span-3">
                                  ...
                                  <div className="mt-1">
                                    <SearchableSelect
                                      ...
                                      placeholder={t(
                                        "expenses.invoices.modal.selectCategory",
                                      )}
                                      searchPlaceholder={t(
                                        "expenses.invoices.modal.searchCategory",
                                      )}
                                      noResultsText={t(
                                        "expenses.invoices.modal.noCategoriesFound",
                                      )}
                                      disabled={isViewing || loadingData}
                                    />
                                  </div>
                                ...
                                <div className="col-span-2">
                                  ...
                                  <div className="mt-1">
                                    <SearchableSelect
                                      options={getUnitsOptionsForCategory(
                                        item.category_id,
                                      )}
                                      value={
                                        item.unit_id
                                          ? getUnitsOptionsForCategory(
                                              item.category_id,
                                            ).find(
                                              (opt) => opt.id === item.unit_id,
                                            ) || null
                                          : null
                                      }
                                      onChange={(
                                      ...
                                      placeholder={t(
                                        "expenses.invoices.modal.selectUnit",
                                      )}
                                      searchPlaceholder={t(
                                        "expenses.invoices.modal.searchUnit",
                                      )}
                                      noResultsText={t(
                                        "expenses.invoices.modal.noUnitsFound",
                                      )}
                                      disabled={
                                        isViewing ||
                                        loadingData ||
                                ...
                                {!isViewing && (
                                  <div className="col-span-1 flex items-end justify-center h-full">
                                    <button
                                      ...
                                      title={t(
                                        "expenses.invoices.modal.removeItem",
                                      )}
                                    >
                        ...
                        <div className="flex justify-end">
                          <div className="text-right">
                            <p className="text-sm text-gray-600">
                              {t("expenses.invoices.modal.totalAmount")}
                            </p>
