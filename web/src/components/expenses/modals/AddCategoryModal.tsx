...
import { expenseCategoriesApi, unitsApi, type ExpenseCategoryCreate, type ExpenseCategory, type Unit } from '~/shared/api';
...
export default function AddCategoryModal({
  ...
  useEffect(() => {
    const loadUnits = async () => {
      ...
      try {
        ...
        setError(t('expenses.modals.addCategory.unitsLoadError'));
      } finally {
  ...
  const handleClose = () => {
    setFormData({ name: '', default_unit_id: 0, order_index: 0 });
    setErrors({});
    setError(null);
    onClose();
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      newErrors.name = t('expenses.modals.addCategory.nameMinLength');
    } else if (formData.name.trim().length > 200) {
      newErrors.name = t('expenses.modals.addCategory.nameMaxLength');
    }

    if (!formData.default_unit_id || formData.default_unit_id <= 0) {
      newErrors.default_unit_id = t('expenses.modals.addCategory.unitRequired');
    }

    if (formData.order_index < 0) {
      newErrors.order_index = 'Порядковый номер должен быть больше или равен 0';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Произошла ошибка при создании категории'
      );
    } finally {
      setIsLoading(false);
    }
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <input
                        ...
                        placeholder="Например: Арабика, Робуста, Молоко 3.2%"
                        autoComplete="off"
                      />
                      {errors.name && (
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      ...
                      >
                        <option value={0}>
                          {isLoadingUnits ? 'Загрузка...' : 'Выберите единицу измерения'}
                        </option>
                        {units.map((unit) => (
                    ...
                    {units.length === 0 && !isLoadingUnits && (
                      <p className="mt-1 text-xs text-amber-600">
                        Нет доступных единиц измерения. Создайте их сначала в настройках.
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Определяет порядок отображения категории. 0 = первая.
                    </p>
                  </div>
